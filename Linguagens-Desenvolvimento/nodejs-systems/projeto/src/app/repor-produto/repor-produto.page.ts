...
salvaReporProduto(){  
    ...
    NotaFiscalDaCompra: this.chaveDeAcesso
  };
...
cancelSale() {
  this.cart = [];
  this.limparCampos(true);  // Não limpa todos os campos, pois não estamos fazendo a confirmação do pedido
  alert('Reposição cancelada!');
}
...
formatarChaveDeAcesso() {
  ...
  // Limita a chave para 54 caracteres no total (44 números + 10 espaços)
  if (valor.length > 44) {
    valor = valor.slice(0, 44);  // A chave de acesso tem 44 dígitos
  }

  // Formata a chave de acesso com espaços a cada 4 caracteres
  let chaveComEspacos = valor.replace(/(\d{4})(?=\d)/g, '$1 ');

  // Agora a chave com espaços pode ter até 54 caracteres (44 números + 10 espaços)
  this.chaveDeAcessoExibicao = chaveComEspacos;

  // Salva a chave de acesso real (com espaços) para ser enviada ao banco
  this.chaveDeAcesso = chaveComEspacos; 
}
